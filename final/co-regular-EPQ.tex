...
%It is a direct consequence of the above definitions. 
...
The execution in \figurename~\ref{fig:introduce gap for EPQ1Lar} is not $\mathsf{MatchedMaxPriority}^>$-linearizable because the left-right constraint of the maximal priority value $b$ contains the cycle {\color{red} $f \rightarrow e \rightarrow d \rightarrow b \rightarrow f$}. The presence of such a cycle is equivalent to \emph{non} $\mathsf{MatchedMaxPriority}^>$-linearizability:
%(see Appendix~\ref{sec:appendix proof and definition in section co-regular of EPQ1Lar}), {\color {red}as indicated by the following lemma:
...
%Given a data-differentiated execution $e$ where 
...
%Let $G$ be the graph representing the left-right constraint of $x$. 
