...
$\seqPQ$ is co-regular, if for each $\Gamma\in \{\mathsf{EmptyRemove}$, $\mathsf{UnmatchedMaxPriority}$ $,\mathsf{MatchedMaxPriority}\}$, there is a finite set $\textit{Auts}_{\Gamma}$ of register automata, such that for each data-independence implementation $\mathcal{I}$, we have that: 
...
The second point is to ignore the incomparable priorities. Given a data-differentiated execution $e$, we say that $e$ is a $p$-execution, if $p$ is larger than or equal to the priority of values of $e$. Given a data-differentiated execution $e$ and assume that $p$ is one of the maximal priority of $e$, let $\textit{p-Exec}(e)$ be an execution obtained from $e$ by erasing all operations of values whose priority are incomparable with $\textit{pri}$. The following lemma states that when checking co-regular of $\seqPQ$, it is safe to consider only $p$-executions.

\begin{restatable}{lemma}{priExecutionIsEnough}
\label{lemma:pri execution is enough}
Given a data-differentiated execution $e$ and $\Gamma\in \{\mathsf{UnmatchedMaxPriority}$ $,\mathsf{MatchedMaxPriority}\}$. $\Gamma\mathsf{\text{-}Conc}(e,x)$ holds, if and only if $\Gamma\mathsf{\text{-}Conc}(\textit{p-Exec}(e),x)$ holds, where $p$ is the priority of $x$.
\end{restatable}
\begin {proof} (Sketch) 
